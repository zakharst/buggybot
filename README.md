...
| `AZURE_DEVOPS_REQUIRED_FIELD_VALUES` | Optional JSON **object** of field ref → value, applied on every create (no WIQL). Set once in Vercel (or `.env`); update when tags/picklists change. Keys for title, description, severity, and assignee are ignored. **Empty strings are dropped** (picklists reject them). **`System.AreaPath`** can be overridden when `AZURE_DEVOPS_TEMPLATE_WORK_ITEM_ID` is set. **`System.IterationPath`** from this JSON is **not sent by default** (ADO / template defaults); it is set only when **iteration team name** is configured (admin or `AZURE_DEVOPS_ITERATION_TEAM_NAME`) and Team Settings returns the current sprint. **Discover fields:** `npm run ado:list-bug-fields`. |
...
| `AZURE_DEVOPS_ITERATION_TEAM_NAME` | **Opt-in** for **`System.IterationPath`**: exact Boards **team** name (Project Settings → Teams). On each create, call [Team Settings iterations](https://learn.microsoft.com/en-us/rest/api/azure/devops/work/iterations/list) **`$timeframe=current`** and set **`System.IterationPath`** to the returned **`path`** (a real tree string). If unset (and no admin iteration team), **`System.IterationPath` is omitted** so ADO / process defaults apply. **Not** the WIQL macro `@CurrentIteration` — that macro is for queries only and triggers **TF401347** on REST create. If the API returns no current sprint, **`System.IterationPath` is removed** so a stale value from `REQUIRED_FIELD_VALUES` does not cause **TF401347**. |
| `AZURE_DEVOPS_DEFAULT_ITERATION` | Set to `1` to **force omit** **`System.IterationPath`** even when **iteration team name** is configured (skips Team Settings lookup and clears any value from `REQUIRED_FIELD_VALUES`). |
| `AZURE_DEVOPS_OMIT_ITERATION_PATH` | Same as **`AZURE_DEVOPS_DEFAULT_ITERATION=1`** (legacy name). |
