...
| `AZURE_DEVOPS_ITERATION_TEAM_NAME` | Optional. Exact Boards **team** name (Project Settings → Teams). On each create, call [Team Settings iterations](https://learn.microsoft.com/en-us/rest/api/azure/devops/work/iterations/list) **`$timeframe=current`** and set **`System.IterationPath`** to the returned **`path`** (a real tree string). **Not** the WIQL macro `@CurrentIteration` — that macro is for queries only and triggers **TF401347** on REST create. If the API returns no current sprint, **`System.IterationPath` is removed** so a stale value from `REQUIRED_FIELD_VALUES` does not cause **TF401347**. |
| `AZURE_DEVOPS_DEFAULT_ITERATION` | Set to `1` to **omit** **`System.IterationPath`** on create so **Azure DevOps / your process template** can apply its own default (or leave the field empty per project rules). Skips Team Settings iteration lookup and clears any value from `REQUIRED_FIELD_VALUES`. |
