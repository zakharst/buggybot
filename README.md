...
| `AZURE_DEVOPS_TEMPLATE_TAG` | If set (e.g. **`production`**), before each create the app loads the **latest changed** Bug whose **Tags** contain that substring (WIQL `CONTAINS`) and **reuses** its `System.AreaPath`, `System.IterationPath`, `System.Tags`, `Microsoft.VSTS.Common.ValueArea`, and every **`Custom.*`** field (plus refs listed in `AZURE_DEVOPS_TEMPLATE_EXTRA_FIELD_REFS`). No separate “validation” step—values are applied like the reference bug. **Title, description, severity, assignee** still come from the Slack flow. `System.State` defaults to **New** unless overridden in `AZURE_DEVOPS_REQUIRED_FIELD_VALUES`. Disable template copy: unset, empty, `0`, `false`, or `-`. |
| `AZURE_DEVOPS_TEMPLATE_EXTRA_FIELD_REFS` | Optional comma-separated field reference names to copy from the template bug when not covered by the built-in allowlist (e.g. org-prefixed fields). |
| `AZURE_DEVOPS_REQUIRED_FIELD_VALUES` | Optional JSON **object** merged **after** the template (same keys **override** template). Keys for title, description, severity, and assignee are ignored. **Discover fields:** `npm run ado:list-bug-fields`. |
